...
4. 当链表数量大于8时，链表的结构会自动转换成红黑树的结构，但是在转换前会先判断数组容量是否小于64，如果<64，则直接对数组扩容，不会直接转换成红黑树
...
确定哪个哨兵执行主从切换，这个过程也叫哨兵的Leader选举，满足两个条件即可
1. 投票数>= 哨兵数/2 + 1，也就是超过半数哨兵支持
2. 票数达到配置文件中的配置的值：`quorum`
